...
@php
    $_bc = [
        ...
        ['link'=>'#','desc'=>'Stock Issuance','icon'=>'none','st'=>false],
        ['link'=>'#','desc'=>'Create','icon'=>'none','st'=>true],
    ];
    $_ch = "Stock Issuance"; // Module Name
@endphp
@section('content')
    ...
    <section class="content">
      <div class="box box-default">
        <div class="box-header with-border">
          <h3 class="box-title">Stock Issuance Info</h3>
          ...
          <div class="row">
            <div class="col-sm-3">
              <div class="form-group" style="display: flex;">
                <a href="{{route('inventory.stockissuance')}}" class="btn btn-block btn-primary"><i class="fa fa-arrow-left"></i> Go Back</a>
              </div>
            </div>
            <div class="col-sm-3">
              <div class="form-group" style="display: flex;">
                      ...
                      <div class="box-body">
                        ...
                        <div class="row">
                          <div class="col-sm-12">
                            <div class="form-group">
                              <label>Remarks</label>
                               <input id="txt_remarks" type="text" class="form-control" name="txt_remarks" placeholder="" required="">
                            </div>
                          </div>
                        </div>
                        <div class="row">
    ...
    <script>
      ...
      $('#tbl_itemlist').DataTable( {
        "columnDefs": [
            ...
            {
                "targets": [ 10 ], // hide cc code col
                "visible": false,
                "searchable": false
            },
            {
                "targets": [ 12 ], // hide scc code col
                "visible": false,
                "searchable": false
            }
        ]
      ...
      function EnterItem_Edit(line)
      {
          ...
          $('input[name="txt_remarks"]').val(data[9]);
          $('select[name="select_costcenter"]').val(data[10]).trigger('change');
          $('select[name="select_subcostcenter"]').val(data[12]).trigger('change');
      ...
      function set_tbl_itemlist()
      {
        ...
        table.row.add([line, part_no, item_code, item_desc, qty, unit_code, unit_desc, cost_price, line_amnt, remarks, cc_code, cc_desc, scc_code, scc_desc, buttons]).draw();

        }
        else if($('#ENTER_ITEM').text() == 'Edit') // edit item
        {
          table.row(selectedRow).data([line, part_no, item_code, item_desc, qty, unit_code, unit_desc, cost_price, line_amnt, remarks, cc_code, cc_desc, scc_code, scc_desc, buttons]).draw();
        }
        else // remove item
        {
      ...
      function clear()
      {
        ...
        $('select[name="select_costcenter"]').val('').trigger('change');
        $('select[name="select_subcostcenter"]').val('').trigger('change');
      ...
      function Save()
      {
        ...
        var data = { 
           ...
           $.ajax({
                  url: '{{asset('inventory/stockissuance/edit')}}/'+rec_num,
                  method: 'POST',
                  data: data,
                  success : function(flag)
                           {
                              ...
                              {
                                console.log(flag);
                                location.href= "{{route('inventory.stockissuance')}}";
                              }
                              else
                              {
