            ...
            <div class="col-md-3">
              <div class="form-group">
                ...
                <select class="form-control select2 select2-hidden-accessible" style="width: 100%;" tabindex="-1" aria-hidden="true" name="select_termsofpayment">
                  ...
                  @if($rechdr->payment_term == $mop->mp_code)
                  <option selected="selected" value="{{$mop->mp_code}}">{{$mop->mp_desc}}</option>
                  @else
          ...
          <div class="row">
            <div class="col-sm-3">
              <div class="form-group" style="display: flex;">
                <a href="{{route('inventory.rpo')}}" class="btn btn-block btn-primary"><i class="fa fa-arrow-left"></i> Go Back</a>
              </div>
            </div>
            <div class="col-sm-3">
              <div class="form-group" style="display: flex;">
                <a class="btn btn-block btn-success" style="margin-top: 0;"  onclick="Save()"><i class="fa fa-save"></i> Save Receiving PO</a>
              </div>
            </div>
          </div>
        </div>
    ...
    <script>
      ...
      $('#tbl_itemlist').DataTable( {
        "columnDefs": [
            ...
            {
                "targets": [ 13 ], // hide cc code col
                "visible": false,
                "searchable": false
            },
            {
                "targets": [ 15 ], // hide scc code col
                "visible": false,
                "searchable": false
            }
        ]
      ...
      function set_tbl_itemlist()
      {
        ...
        table.row.add([line, part_no, item_code, item_desc, qty, unit_code, unit_desc, cost_price, disc_amnt, line_amnt, net_price, line_vat, vat_amt, cc_code, cc_desc, scc_code, scc_desc, buttons]).draw();

        }
        else if($('#ENTER_ITEM').text() == 'Edit') // edit item
        {
          table.row(selectedRow).data([line, part_no, item_code, item_desc, qty, unit_code, unit_desc, cost_price, disc_amnt, line_amnt, net_price, line_vat, vat_amt, cc_code, cc_desc, scc_code, scc_desc, buttons]).draw();
        }
        else // remove item
        {
      ...
      function Save()
      {
        ...
        var data = { 
           ...
           $.ajax({
                  url: '{{asset('inventory/receiving_po/rpo_edit')}}/'+rec_num,
                  method: 'POST',
                  data: data,
