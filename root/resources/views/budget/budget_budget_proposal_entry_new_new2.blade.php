...
@php
    $_bc = [
        ...
        ['link'=>url("budget/budget-proposal-entry-new/new"),'desc'=>'New','icon'=>'none','st'=>true]
    ];
    ...
    <div class="box box-default">
        ...
        <div class="box-body" style="">
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        <label>Financial Period <span style="color:red"><strong>*</strong></span></label>
...
<script>
		...
		if($('#ItmForm').parsley().validate())
		{
			if($('#MOD_MODE').text() == '(ADD)')
			{
				table.row.add([
						...
						'<center>' +
							'<a class="btn btn-social-icon btn-warning"  data-toggle="modal" data-target="#modal-default"><i class="fa fa-pencil" onclick="EditMode(\''+line+'\', \''+acc_title_id+'\', \''+parseFloat(amt)+'\', \''+subgrpid+'\');"></i></a>&nbsp;' +
	...
	function EditMode(line, acc_title_id, amt, subgrpid)
	{
        // alert(acc_title_id);
		...
		if($('input[name="hdr_bgtps01"]').val() != '')
        {
            if($('#HdrForm').parsley().validate())
            {
                if(($('.table').DataTable().data().count()/ 6) != 0)
                {
                    ...
                    $.ajax({
                        url : '{{ url('budget/budget-proposal-entry-new/save') }}',
                        method : 'POST',
                        data : data,
                        success : function(d){
                            if(d == 'DONE'){
