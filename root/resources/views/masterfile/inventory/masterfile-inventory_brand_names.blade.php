...
@php
    $_bc = [
        ...
        ['link'=>url("master-file/inventory/brand_name"),'desc'=>'Brand Name','icon'=>'none','st'=>true]
    ];
    $_ch = "Brand Names"; // Module Name
@endphp
@section('content')
        						...
        						<div class="modal-dialog">
        							<div class="modal-content">
        								<div class="modal-header">
                                            ...
                                            <h3 class="modal-title">Brand Name <span id="MOD_MODE"></span></h3>
                                        </div>
                                	...
                                	@isset($brand)
                                        @foreach($brand as $fund)
                                            <tr>
                                                ...
                                                <td>
                                                    <center>
                                                       <a class="btn btn-social-icon btn-warning" data-toggle="modal" data-target="#modal-default"><i class="fa fa-pencil" onclick="EditMode('{{$fund->brd_code}}', '{{$fund->brd_name}}');"></i></a>
                                                       <a class="btn btn-social-icon btn-danger" data-toggle="modal" data-target="#modal-default" onclick="DeleteMode('{{$fund->brd_code}}', '{{$fund->brd_name}}');"><i class="fa fa-trash "></i></a>
                                                    </center>
                                                </td>
                                            </tr>
                                        @endforeach
	...
	<script type="text/javascript">
        ...
        {
            $('#MOD_MODE').text('(New)');
            $('#AddForm').attr('action', '{{ url('master-file/inventory/brand_name') }}');
        ...
        {
            $('#MOD_MODE').text('(Edit)');
            $('#AddForm').attr('action', '{{ url('master-file/inventory/brand_name') }}/update');
        ...
        function DeleteMode(id, desc)
        {
            ...
            $('#AddForm').attr('action', '{{ url('master-file/inventory/brand_name') }}/delete');
            $('input[name="txt_id"]').val(id);
