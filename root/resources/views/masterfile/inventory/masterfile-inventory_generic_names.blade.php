...
@php
    $_bc = [
        ...
        ['link'=>url("master-file/inventory/generic_name"),'desc'=>'Generic Name','icon'=>'none','st'=>true]
    ];
    $_ch = "Generic Names"; // Module Name
@endphp
@section('content')
        		...
        		<div class="col-xs-12">
        			<div class="box">
        				<div class="box-header">
        					<h3 class="box-title">Generic Name List</h3>
        						...
        						<div class="modal-dialog">
        							<div class="modal-content">
        								<div class="modal-header">
                                            ...
                                            <h3 class="modal-title">Generic Name <span id="MOD_MODE"></span></h3>
                                        </div>
        				...
        				<div class="box-body">
        					<table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                	@isset($generic)
                                        @foreach($generic as $fund)
                                            <tr>
                                                <th>{{$fund->gen_code}}</th>
                                                <td>{{$fund->gen_name}}</td>
                                                <td>
                                                    <center>
                                                       <a class="btn btn-social-icon btn-warning" data-toggle="modal" data-target="#modal-default"><i class="fa fa-pencil" onclick="EditMode('{{$fund->gen_code}}', '{{$fund->gen_name}}');"></i></a>
                                                       <a class="btn btn-social-icon btn-danger" data-toggle="modal" data-target="#modal-default" onclick="DeleteMode('{{$fund->gen_code}}', '{{$fund->gen_name}}');"><i class="fa fa-trash "></i></a>
                                                    </center>
                                                </td>
                                            </tr>
                                        @endforeach
	...
	<script type="text/javascript">
        ...
        {
            $('#MOD_MODE').text('(New)');
            $('#AddForm').attr('action', '{{ url('master-file/inventory/generic_name') }}');
        ...
        {
            $('#MOD_MODE').text('(Edit)');
            $('#AddForm').attr('action', '{{ url('master-file/inventory/generic_name') }}/update');
        ...
        function DeleteMode(id, desc)
        {
            ...
            $('#AddForm').attr('action', '{{ url('master-file/inventory/generic_name') }}/delete');
            $('input[name="txt_id"]').val(id);
