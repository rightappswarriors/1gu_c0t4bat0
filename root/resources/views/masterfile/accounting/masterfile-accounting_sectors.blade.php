...
@php
    $_bc = [
        ...
        ['link'=>url("master-file/accounting/sector"),'desc'=>'Sector','icon'=>'none','st'=>true]
    ];
    $_ch = "Sectors"; // Module Name
@endphp
@section('content')
        			...
        			<div class="box">
        						...
        						<div class="modal-dialog">
        							<div class="modal-content">
        								<div class="modal-header">
                                            ...
                                            <h3 class="modal-title">Sectors <span id="MOD_MODE"></span></h3>
                                        </div>
        				...
        				<div class="box-body">
        					<table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                	@isset($sectors)
                                        @foreach($sectors as $fund)
                                            <tr>
                                                <th>{{$fund->secid}}</th>
                                                <td>{{$fund->secdesc}}</td>
                                                <td>
                                                    <center>
                                                       <a class="btn btn-social-icon btn-warning" data-toggle="modal" data-target="#modal-default"><i class="fa fa-pencil" onclick="EditMode('{{$fund->secid}}', '{{$fund->secdesc}}');"></i></a>
                                                       <a class="btn btn-social-icon btn-danger" data-toggle="modal" data-target="#modal-default" onclick="DeleteMode('{{$fund->secid}}', '{{$fund->secdesc}}');"><i class="fa fa-trash "></i></a>
                                                    </center>
                                                </td>
                                            </tr>
                                        @endforeach
	...
	<script type="text/javascript">
        ...
        {
            $('#MOD_MODE').text('(New)');
            $('#AddForm').attr('action', '{{ url('master-file/accounting/sector') }}');
        ...
        {
            $('#MOD_MODE').text('(Edit)');
            $('#AddForm').attr('action', '{{ url('master-file/accounting/sector') }}/update');
        ...
        function DeleteMode(id, desc)
        {
            ...
            $('#AddForm').attr('action', '{{ url('master-file/accounting/sector') }}/delete');
            $('input[name="txt_id"]').val(id);
