...
@php
    $_bc = [
        ...
        ['link'=>url("master-file/Miscellaneous/bank"),'desc'=>'Bank','icon'=>'none','st'=>true]
    ];
...
@section('content')
                                                    ...
                                                    <div class="box-body">
                                                        <div class="form-group">
                                                            ...
                                                            <div class="col-sm-8" style="margin-bottom:10px;">
                                                                <input readonly="" type="text" name="txt_id" class="form-control" placeholder="id" data-parsley-required-message="<strong>id</strong> is required." required>
                                                            </div>
                                                        </div>
                                                        <div class="form-group">
                                	...
                                	@isset($bank)
                                        @foreach($bank as $rp)
                                            <tr>
                                                ...
                                                <td>
                                                    <center>
                                                       <a class="btn btn-social-icon btn-warning" data-toggle="modal" data-target="#modal-default"><i class="fa fa-pencil" onclick="EditMode('{{$rp->b_code}}', '{{$rp->b_name}}');"></i></a>
                                                       <a class="btn btn-social-icon btn-danger" data-toggle="modal" data-target="#modal-default" onclick="DeleteMode('{{$rp->b_code}}', '{{$rp->b_name}}');"><i class="fa fa-trash "></i></a>
                                                    </center>
                                                </td>
                                            </tr>
                                        @endforeach
	...
	<script type="text/javascript">
        ...
        {
            $('#MOD_MODE').text('(New)');
            $('#AddForm').attr('action', '{{ url('master-file/Miscellaneous/bank') }}');
        ...
        {
            $('#MOD_MODE').text('(Edit)');
            $('#AddForm').attr('action', '{{ url('master-file/Miscellaneous/bank') }}/update');
        ...
        function DeleteMode(id, desc)
        {
            ...
            $('#AddForm').attr('action', '{{ url('master-file/Miscellaneous/bank') }}/delete');
            $('input[name="txt_id"]').val(id);
            $('input[name="txt_id"]').attr('required');
            $('input[name="txt_name"]').val(desc);
            $('input[name="txt_name"]').attr('required', '');
