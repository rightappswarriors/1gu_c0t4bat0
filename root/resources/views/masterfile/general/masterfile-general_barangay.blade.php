...
@php
    $_bc = [
        ...
        ['link'=>'#','desc'=>'General','icon'=>'none','st'=>false],
        ['link'=>url("master-file/general/barangay"),'desc'=>'Barangay','icon'=>'none','st'=>true]
    ];
    $_ch = "Barangay"; // Module Name
@endphp
@section('content')
        		...
        		<div class="col-xs-12">
        			<div class="box">
        				<div class="box-header">
        					<h3 class="box-title">Barangay List</h3>
        						...
        						<div class="modal-dialog">
        							<div class="modal-content">
        								<div class="modal-header">
                                            ...
                                            <h3 class="modal-title">Barangay <span id="MOD_MODE"></span></h3>
                                        </div>
        				...
        				<div class="box-body">
        					<table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                	@isset($brand)
                                        @foreach($brand as $fund)
                                            <tr>
                                                <th>{{$fund->brgy_id}}</th>
                                                <td>{{$fund->brgy_name}}</td>
                                                <td>
                                                    <center>
                                                       <a class="btn btn-social-icon btn-warning" data-toggle="modal" data-target="#modal-default"><i class="fa fa-pencil" onclick="EditMode('{{$fund->brgy_id}}', '{{$fund->brgy_name}}');"></i></a>
                                                       <a class="btn btn-social-icon btn-danger" data-toggle="modal" data-target="#modal-default" onclick="DeleteMode('{{$fund->brgy_id}}', '{{$fund->brgy_name}}');"><i class="fa fa-trash "></i></a>
                                                    </center>
                                                </td>
                                            </tr>
                                        @endforeach
	...
	<script type="text/javascript">
        $(document).ready(function(){
            ...
            // $('#SideBar_MFile_BRAND_NAME').addClass('text-green');
        ...
        {
            $('#MOD_MODE').text('(New)');
            $('#AddForm').attr('action', '{{ url('master-file/general/barangay') }}');
        ...
        {
            $('#MOD_MODE').text('(Edit)');
            $('#AddForm').attr('action', '{{ url('master-file/general/barangay') }}/update');
        ...
        function DeleteMode(id, desc)
        {
            ...
            $('#AddForm').attr('action', '{{ url('master-file/general/barangay') }}/delete');
            $('input[name="txt_id"]').val(id);
