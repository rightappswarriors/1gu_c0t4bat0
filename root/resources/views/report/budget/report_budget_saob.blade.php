                        ...
                        <form id="ItmForm" novalidate data-parsley-validate>
                            <span class="AddMode">
                                ...
                                <div class="row">
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            <label>Month <span style="color:red"><strong>*</strong></span></label>
                                            <select class="form-control select2 select2-hidden-accessible" name="mo_from" value="" style="width: 100%;" data-parsley-errors-container="#mo_from" tabindex="-1" aria-hidden="true" required="" data-parsley-required-message="<strong>Month</strong> is required.">
                                                <option value disabled hidden selected>Select Month (from)</option>
                                                <option value="1">January</option>
                                                <option value="7">July</option>
                                            </select>
                                            <span id="mo_from"></span>
                                        </div>
                                    </div>
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            <label>Month <span style="color:red"><strong>*</strong></span></label>
                                            <select class="form-control select2 select2-hidden-accessible" name="mo_to" value="" style="width: 100%;" data-parsley-errors-container="#mo_to" tabindex="-1" aria-hidden="true" required="" data-parsley-required-message="<strong>Month</strong> is required.">
                                                <option value disabled hidden selected>Select Month (to)</option>
...
<script>
    ...
    function GenerateSaob()
    {
        if($('#ItmForm').parsley().validate())
        {
            ...
            var mo_from = $('select[name="mo_from"]').val();
            var mo_to = $('select[name="mo_to"]').val();
            // var test = fymo.split('-');
            // var fy = test[0];
            // var mo = test[1];
            // + sector + "_"
            location.href = "{{ url('reports/budget/saaob/') }}/" + fid + "_" + budget_period + "_" + mo_from + "_" + mo_to;
        }
    }
</script>
@endsection