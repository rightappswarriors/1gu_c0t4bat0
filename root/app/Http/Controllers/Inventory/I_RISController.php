...
class I_RISController extends Controller
{
    ...
    public function add_02(Request $request)
    {
        if($request->isMethod('get'))
        {
            ...
            return view('inventory.ris.ris-entry-02', compact('stock_loc', 'branch', 'disp_items', 'itemunit', 'costcenter', 'vat', 'isnew', 'x08', 'are_users', 'are_position', 'toolbar', 'stockin'));
        }
        else if($request->isMethod('post'))
        {
            $datetime = Carbon::now();
    ...
    public function edit_02(Request $request, $code)
    {
      ...
      if($request->isMethod('get')) // for entry
      {
          ...
          return view('inventory.ris.ris-entry-02', compact('rechdr', 'reclne', 'stock_loc', 'branch', 'itemunit', 'costcenter', 'vat', 'disp_items', 'grandtotal', 'isnew', 'x08', 'are_users', 'are_position', 'stockin', 'toolbar'));
      }
      elseif($request->isMethod('post'))
      {
          $datetime = Carbon::now();
